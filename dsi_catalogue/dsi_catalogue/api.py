...
def create_website_route_meta(route, seo_data):
	...
	if frappe.db.exists("Website Route Meta", route):
		...
		doc = frappe.get_doc(
			{"doctype": "Website Route Meta", "name": route, "__newname": route, "meta_tags": meta_tags}
		)
		doc.insert(ignore_permissions=True)
...
def create_website_item(item_code, catalogue, content):
	...
	if images:
		...
		doc.website_image = create_file_for_external_url(
			image_url, doctype="Website Item", docname=doc.name, filename=images[0].get("fileName")
		)
		doc.save(ignore_permissions=True)
	...
	if specs or care:
		...
		for label, value in specs.items():
			doc.append("website_specifications", {"label": str(label), "description": str(value)})
		if care:
			doc.append("website_specifications", {"label": "Care Instructions", "description": care})
		doc.save(ignore_permissions=True)
...
def receive_publish_callback(
	...
	item_group_data = (
		json.loads(item_group_data) if isinstance(item_group_data, str) else (item_group_data or {})
	)
	...
	item_group = None
	if item_group_data:
		item_group = get_or_create_item_group(item_group_data.get("palace"), item_group_data.get("range"))
		...
		doc = frappe.get_doc(
			{
				"doctype": "Website Item",
				"item_code": item_code,
				"web_item_name": content.get("product_name")
				or (catalogue.display_name if catalogue else item_code),
				"published": 1,
				"item_group": item_group,
			}
		)
...
def get_publish_status(task_id):
	...
	return {"task_id": task_id, "status": "processing", "message": "Content generation in progress..."}
...
def receive_generation_callback(
	task_id, content, images=None, seo_data=None, item_group_data=None, status="success", error=None
):
	...
	if status != "success":
		...
		frappe.cache.set_value(
			cache_key,
			{"status": "error", "error": error or content.get("error", "Unknown error")},
			expires_in_sec=GENERATION_CACHE_TTL,
		)
		return {"success": False}
...
def get_website_item_by_index_key(index_key):
	...
	items = frappe.get_all(
		"Website Item",
		filters={"published": 1, "custom_index_key": index_key},
		fields=[
			"name",
			"item_code",
			"web_item_name",
	...
	specs = frappe.get_all(
		"Website Item Website Specification", filters={"parent": item.name}, fields=["label", "description"]
	)
...
def publish_website_item(folder_id, item_code, content, images=None, seo_data=None, item_group_data=None):
	...
	try:
		...
		if images:
			...
			image_url = (
				primary_img.get("cloudinaryUrl") or primary_img.get("url") or primary_img.get("web_optimized")
			)
			# Create a File record for the external URL so ERPNext can reference it
			doc.website_image = create_file_for_external_url(
				image_url, doctype="Website Item", docname=doc.name, filename=primary_img.get("fileName")
			)
			# Store alt text in image alt field if available
			if hasattr(doc, "website_image_alt"):
				doc.website_image_alt = primary_img.get("alt") or content.get("image_alts", {}).get(
					primary_img.get("fileName"), ""
				)
		...
		if images:
			...
			image_alts = content.get("image_alts", {})
			doc.custom_image_alt_text = image_alts.get(first_image_name, "") or primary_img.get(
				"description", ""
			)
		...
		if image_alts:
			doc.append(
				...
				{"label": "Image Descriptions", "description": json.dumps(image_alts)},
			)
...
def get_product_images_by_index_key(index_key_prefix=None):
	...
	website_items = frappe.get_all(
		...
		fields=["name", "item_code", "web_item_name", "website_image", "slideshow", "custom_index_key"],
	...
	for idx, wi in enumerate(website_items):
		...
		for fi_idx, fi in enumerate(file_images):
			...
			if (
				url
				and url.lower().endswith((".png", ".jpg", ".jpeg", ".webp", ".gif"))
	...
	catalogue_items = [
		...
		if (
			_ik.get_product_grouping_key(c.index_key)
			or _ik.get_template_index_key(c.index_key)
			or c.index_key
		)
		== grouping
	]
	for cat in catalogue_items:
		...
		if cat.cloudinary_images:
			try:
				cloud_imgs = (
					json.loads(cat.cloudinary_images)
					if isinstance(cat.cloudinary_images, str)
					else cat.cloudinary_images
				)
...
def get_product_gallery(item_code=None, index_key=None):
	...
	if not key and item_code:
		key = frappe.db.get_value(
			"Website Item", {"item_code": item_code}, "custom_index_key"
...
def get_shop_filters(palace=None):
	...
	items = frappe.get_all("Website Item", filters={"published": 1}, fields=["custom_index_key"])
