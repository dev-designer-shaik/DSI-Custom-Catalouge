...
def capture_web_lead(payload=None):
	...
	if cart_value is None:
		cart_value = sum(float(i.get("price") or 0) * float(i.get("qty") or 1) for i in items)
	...
	if not existing and email:
		...
		existing = frappe.db.get_value("Lead", {"email_id": email.strip()}, "name")
	...
	fields = {
		...
		**(
			{}
			if not payload.get("typed_address")
			else {"typed_address_json": json.dumps(payload["typed_address"], ensure_ascii=False)}
		),
	...
	if data_source == CART_SOURCE:
		...
		lead_name = " ".join(x for x in [payload.get("first_name"), payload.get("last_name")] if x) or (
			email.split("@")[0] if email else "Website inquirer"
		)
