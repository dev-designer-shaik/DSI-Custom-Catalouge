		...
		out.append(
			{
				...
				"title": wi.get("web_item_name"),
				"grouping": grp,
				"palace": wi.get("custom_palace_slug"),
				"l2": l2,
				"l3": l3,
				"actions": actions,
			}
...
def forward_fill_media(item_code=None, grouping=None, dry_run=1):
	...
	wi = (
		frappe.db.get_value(
			"Website Item",
			{"item_code": item_code} if item_code else {"name": grouping},
			["name", "custom_grouping_key", "custom_index_key", "website_image"],
			as_dict=True,
		)
		if (item_code or grouping)
		else None
	)
	if not wi:
	...
	result["changes"].append({"field": "custom_gallery_images", "from_count": cur_gal, "to_count": len(imgs)})
	if not dry_run:
		if not wi.website_image:
			frappe.db.set_value(
				"Website Item", wi.name, "website_image", hero["image"], update_modified=False
			)
		...
		if r.get("changes"):
			touched += 1
			done.append(
				{
					...
					"imgs": r["catalogue_images"],
					"changes": [c["field"] for c in r["changes"]],
				}
			)
	return {"scanned": len(wis), "with_changes": touched, "dry_run": bool(dry_run), "items": done}
...
def backward_fill(item_code=None, website_item=None, dry_run=1):
	...
	wi = (
		frappe.db.get_value(
			"Website Item",
			{"item_code": item_code} if item_code else {"name": website_item},
			[
				"name",
				"item_code",
				"web_item_name",
		...
		if (item_code or website_item)
		else None
	)
	if not wi:
		frappe.throw("Website Item not found")
	specs = frappe.get_all(
		"Item Website Specification", filters={"parent": wi.name}, fields=["label", "description"]
	)
	curated = {
	...
	if not any(
		[
			curated["website_content"],
			curated["web_long_description"],
	...
	like = grp.rstrip("}")
	cats = frappe.get_all("Product Catalogue", filters=[["index_key", "like", like + "%"]], pluck="name")
	target = cats[0] if cats else None
	created = False
	result = {"website_item": wi.name, "grouping": grp, "applied": False}
	if not target:
		folder_id = f"PENDING:{wi.custom_index_key}"
			...
			doc = frappe.get_doc(
				{
					...
					"folder_id": folder_id,
					"index_key": wi.custom_index_key,
					"display_name": wi.web_item_name,
					"ai_metadata": json.dumps({"curated_marketing": curated, "_pending_drive_sync": True}),
				}
			)
		...
		if not dry_run:
			...
			frappe.db.set_value(
				"Product Catalogue", target, "ai_metadata", json.dumps(md), update_modified=False
			)
	if not dry_run:
		frappe.db.commit()
		result["applied"] = True
...
def seed_catalogue_from_items(item_codes=None, dry_run=1):
	...
	for ic in item_codes:
		wi = frappe.db.get_value(
			"Website Item",
			{"item_code": ic},
			["web_item_name", "custom_index_key", "website_image"],
			as_dict=True,
		)
		if not wi or not wi.website_image or not wi.custom_index_key:
			skipped.append(ic)
			continue
		folder_id = f"WISEED:{wi.custom_index_key}"
			...
			frappe.get_doc(
				{
					...
					"hero_image": img,
					"cloudinary_images": json.dumps(cloud),
					"ai_metadata": json.dumps({"_seeded_from_website_item": True}),
				}
			).insert(ignore_permissions=True)
	if not dry_run:
		frappe.db.commit()
	return {"created": len(created), "skipped": len(skipped), "dry_run": bool(dry_run), "items": created}
