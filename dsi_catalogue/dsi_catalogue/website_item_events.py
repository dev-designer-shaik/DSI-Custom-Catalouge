...
def _i18n_restore_dropped_rows(doc, prev):
	...
	for lang in missing:
		...
		payload = {
			k: v
			for k, v in (src.as_dict() if hasattr(src, "as_dict") else dict(src)).items()
			if k
			not in (
				"name",
				"idx",
	...
	try:
		...
		if lang != "en":
			if changed:
				...
				frappe.msgprint(
					frappe._(
						"Your {0} edits were saved as a translation and the English copy was restored."
					).format(lang),
					indicator="orange",
				)
		...
		if from_form and not has_beacon and changed:
			frappe.throw(
				frappe._(
					"The translation editor did not load. Reload this page (Ctrl+Shift+R) before editing product copy."
				),
...
def _i18n_reroute_to_translation(doc, language, changed):
	...
	if row is None:
		row = doc.append(
			"custom_translations",
			{"language": language, "review_status": "Machine (unreviewed)", "source": "Human"},
		)
	...
	try:
		...
		for row in doc.get("custom_translations") or []:
			...
			if int(row.get("is_stale") or 0) != stale:
				frappe.db.set_value(
					"Website Item Translated Copy", row.name, "is_stale", stale, update_modified=False
				)
	except Exception:
