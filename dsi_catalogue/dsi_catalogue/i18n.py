...
def _row_name(website_item, language):
    rows = frappe.get_all(
        CHILD_DOCTYPE,
        filters={"parent": website_item, "parenttype": "Website Item",
                 "parentfield": PARENTFIELD, "language": language},
...
@frappe.whitelist()
def put_copy(website_item, language, payload, base_en_fingerprint=None, row_modified=None, skip_revalidate=False):
    ...
    frappe.db.set_value("Website Item", website_item, "modified", frappe.utils.now(),
                        update_modified=False)
    frappe.db.commit()

    if not skip_revalidate:
        _notify_revalidate(website_item)
    return {"ok": True, "row_name": row_name,
...
def restore_english(website_item):
    ...
    for row in bundle.get("specifications") or []:
        if row.get("name") and frappe.db.exists("Item Website Specification", row["name"]):
            frappe.db.set_value("Item Website Specification", row["name"],
