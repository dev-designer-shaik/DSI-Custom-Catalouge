...
with open(_MAPS_PATH, encoding="utf-8") as _f:
	_M = json.load(_f)
...
def slugify(text):
	text = (text or "").lower()
	text = re.sub(r"['‘’]", "", text)  # noqa: RUF001  straight + curly apostrophes are the point
...
def decode_index_key(index_key):
	...
	if len(parts) > product_start + 1:
		compound = f"{parts[product_start]}-{parts[product_start + 1]}"
...
def get_template_index_key_for_product(index_key):
	...
	return build_index_key(
		d["palace"]["code"], (d["range"] or {}).get("code"), d["productCode"], template_variants
	)
